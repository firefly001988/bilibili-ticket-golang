...
```mermaid
flowchart LR
    User[用户 / Wails UI] --> Planner[Planner<br/>生成 Intent]
    Planner --> Dispatcher[Dispatcher<br/>分配账号与 Worker]
    Dispatcher --> DB[(employer.db)]
    Dispatcher -->|Submit / Status / Stop<br/>gRPC + mTLS| Worker[本机或远程 Worker]
    Worker --> Engine[执行引擎]
    Engine -->|循环下单| Bili[Bilibili API]
    Worker -->|Heartbeat 完成事件| Dispatcher
    DB --> User
```

桌面端持有规划和调度状态, Worker 只接收不可变的 `ExecutionSpec` 并执行. 业务状态以雇主端 SQLite 为准, Worker 的本地成功索引用于保证成功结果在报告前已经持久化.

## 进程与文件

- `bilibili-ticket-golang`: Wails v3 雇主桌面应用, SQLite 持久化, 规划器 (Planner), 调度器 (Dispatcher).
...
| `Heartbeat` | 双向流: Worker 定期发送心跳, 雇主回显确认. 任一端超时未收到消息则视为断连, 触发自动重连. 心跳消息携带当前活动 Attempt ID, 也可携带终态执行结果. |
...
雇主启动时自动检测 `data/local-worker/` 下的 TLS 证书; 不存在则生成自签名 CA 和服务端/客户端证书 (ECDSA P-256). 本机 Worker 的 ID 固定为 `local`, 监听 `127.0.0.1:37900`, 不可手动删除或断开.
...
- 同一 Intent 可用不同账号和 Worker 创建多个副本 (由 `desiredReplicas` 控制); 第一个确认成功的订单赢得逻辑 Intent, 兄弟副本立即被 Dispatcher 停止. 停止生效前已经成功的其他副本仍按独立订单记录和提示.
...
| HTTP 412 | 当前 Worker 冷却 5 分钟, 期间改用其他健康 Worker. |
| 验证码失败 | 作为可恢复失败重试或故障转移, 不对 Worker 设置 5 分钟冷却. |
