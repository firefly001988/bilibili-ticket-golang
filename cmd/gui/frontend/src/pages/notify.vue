...
const currentTypeMeta = computed<NotifyChannelTypeMeta | undefined>(() =>
    channelTypes.value.find((t: { type: string }) => t.type === formType.value),
)

/** v-select items: [{ title: 'Gotify', value: 'gotify' }, ...] */
const typeItems = computed(() =>
    channelTypes.value.map((t: { label: any; type: any }) => ({ title: t.label, value: t.type })),
)
...
function typeLabel(t: string): string {
    const meta = channelTypes.value.find((m: { type: string }) => m.type === t)
    return meta?.label ?? t
}
...
const visibleFields = computed(() => {
    ...
    return currentTypeMeta.value.fields.filter((f: any) => isFieldVisible(f))
})
                        ...
                        <template v-if="currentTypeMeta">
                            <v-col v-for="field in visibleFields" :key="field.key" cols="12">
                                ...
                                <v-select v-if="field.type === 'select'" v-model="formParams[field.key]"
                                    ...
                                    :items="(field.options ?? []).map((o: { label: any; value: any }) => ({ title: o.label, value: o.value }))"
