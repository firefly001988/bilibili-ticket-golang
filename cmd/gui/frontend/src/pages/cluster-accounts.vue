        ...
        <v-table density="compact">
          ...
          <tbody>
            <tr v-for="item in snapshot.accounts" :key="item.id">
              ...
              <td>
                <v-btn size="small" variant="tonal" prepend-icon="mdi-account-sync" @click="syncBuyers(item.id)">同步</v-btn>
                <v-btn class="ml-1" size="small" variant="text" color="error" icon="mdi-delete" @click="deleteAccount(item.id, item.name || item.id)" />
              </td>
            </tr>
          </tbody>
        </v-table>
