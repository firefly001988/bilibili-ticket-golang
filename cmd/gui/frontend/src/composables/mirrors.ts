...
/** Build a select-friendly options array with {title, value} shape (value = index into MIRRORS). */
export function mirrorSelectOptions() {
    return MIRRORS.map((m, i) => ({ title: m.title, value: i }))
}

/** Build a v-select compatible options array with {title, value} shape where value = prefix string. */
