...
type Jar struct {
	...
	entries map[string]map[string]CookieEntries
}
...
func newAtTime(o *Options, now time.Time) *Jar {
	jar := &Jar{
		entries: make(map[string]map[string]CookieEntries),
	}
	if o == nil {
		o = &noOptions
	}
...
func (e *CookieEntries) id() string {
...
// cookie is expired.
func (e *CookieEntries) shouldSend(https bool, host, path string) bool {
	return e.domainMatch(host) && e.pathMatch(path) && (https || !e.Secure)
}

// domainMatch implements "domain-match" of RFC 6265 section 5.1.3.
func (e *CookieEntries) domainMatch(host string) bool {
	if e.Domain == host {
		return true
	}
...
func (e *CookieEntries) pathMatch(requestPath string) bool {
...
// byPathLength is a []CookieEntries sort.Interface that sorts according to RFC 6265
// section 5.4 point 2: by longest path and then by earliest creation time.
type byPathLength []CookieEntries
...
func (s byPathLength) Less(i, j int) bool {
	...
	if !time.Unix(e0.Creation, 0).Equal(time.Unix(e1.Creation, 0)) {
		return time.Unix(e0.Creation, 0).Before(time.Unix(e1.Creation, 0))
	}
...
func (j *Jar) cookies(u *url.URL, now time.Time) (cookies []*http.Cookie) {
	...
	var selected []CookieEntries
	for id, e := range submap {
		if !time.Unix(e.Expires, 0).After(now) {
...
func (j *Jar) RemoveCookie(c *http.Cookie) {
	...
	if e, ok := j.entries[key][id]; ok {
		e.Value = ""
		e.Expires = time.Now().Add(-1 * time.Second).Unix()
		j.entries[key][id] = e
	}
}
...
func (j *Jar) merge(entries []CookieEntries) {
	for _, e := range entries {
		...
		submap := j.entries[key]
		if submap == nil {
			j.entries[key] = map[string]CookieEntries{
				id: e,
			}
			continue
		}
		oldEntry, ok := submap[id]
		if !ok || time.Unix(e.Updated, 0).After(time.Unix(oldEntry.Updated, 0)) {
			submap[id] = e
		}
	}
}
...
func (j *Jar) newEntry(c *http.Cookie, now time.Time, defPath, host string) (e CookieEntries, err error) {
	e.Name = c.Name
	if c.Path == "" || c.Path[0] != '/' {
		e.Path = defPath
	} else {
	...
	if c.MaxAge != 0 {
		e.Persistent = true
		e.Expires = now.Add(time.Duration(c.MaxAge) * time.Second).Unix()
		if c.MaxAge < 0 {
			return e, nil
		}
	} else if c.Expires.IsZero() {
		e.Expires = endOfTime.Unix()
	} else {
		e.Persistent = true
